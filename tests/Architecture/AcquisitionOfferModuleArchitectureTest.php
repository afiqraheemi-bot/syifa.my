...
final class AcquisitionOfferModuleArchitectureTest extends TestCase
{
    public function test_commercial_service_provider_is_registered(): void
    {
        ...
        self::assertStringContainsString('AcquisitionOfferServiceProvider::class', $providers);
    }

    public function test_commercial_offer_is_the_only_aggregate_root(): void
    {
        self::assertFileExists(dirname(__DIR__, 2).'/app/Modules/AcquisitionOffer/Domain/CommercialOffer.php');
        self::assertDirectoryDoesNotExist(dirname(__DIR__, 2).'/app/Modules/AcquisitionOffer/Domain/Aggregates');

        foreach ($this->phpFilesIn(dirname(__DIR__, 2).'/app/Modules/AcquisitionOffer/Domain') as $file) {
    ...
    {
        $migration = $this->source(dirname(__DIR__, 2).'/database/migrations/acquisition_offer/2026_07_21_000001_create_commercial_offer_tables.php');
    ...
    {
        foreach ($this->phpFilesIn(dirname(__DIR__, 2).'/app/Modules/AcquisitionOffer') as $file) {
    ...
    public function test_domain_and_application_are_framework_independent(): void
    {
        foreach ($this->phpFilesIn(
            dirname(__DIR__, 2).'/app/Modules/AcquisitionOffer/Domain',
            dirname(__DIR__, 2).'/app/Modules/AcquisitionOffer/Application',
            dirname(__DIR__, 2).'/app/Modules/AcquisitionOffer/Contracts',
        ) as $file) {
            $source = $this->source($file);
    ...
    {
        foreach ($this->phpFilesIn(dirname(__DIR__, 2).'/app/Modules/AcquisitionOffer/Presentation') as $file) {
            $source = $this->source($file);
    ...
    {
        $routes = $this->source(dirname(__DIR__, 2).'/app/Modules/AcquisitionOffer/Infrastructure/routes/acquisition_offer.php');
