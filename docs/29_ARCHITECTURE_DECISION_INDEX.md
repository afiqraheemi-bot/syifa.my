...
| [ADR-024](./decisions/ADR-024-Public-Website-Delivery-Contract-V1.md) | Public Website Delivery Contract V1 | Active | Authorizes trusted public context, governed routes, Asset/action URL resolution, document delivery, SEO delivery, and platform legal authority after immutable rendering. |
...
| [ADR-032](./decisions/ADR-032-Billing-Renewal-Domain.md) | Billing Renewal Domain | Active, architecture only | Defines one-aggregate Subscription renewal, normalized renewal-to-Payment lineage, manual and automatic renewal, idempotency/concurrency/audit/timeline boundaries, Billing contracts, and Phase 2 Invoice deferral without implementing runtime behavior. |
| [ADR-033](./decisions/ADR-033-Renewal-Commercial-Offer-Resolution.md) | Renewal Commercial Offer Resolution | Active, architecture only | Defines SystemCommercialActor offer ownership, catalogue resolution behind primitive cross-module contracts, staged local transactions, durable renewal checkout orchestration and provider-neutral PaymentSession delivery; narrowly refines ADR-032's request transaction. |
