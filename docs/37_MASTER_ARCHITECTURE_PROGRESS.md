...
| Payment integration for Booking | **Not implemented** | No payment code exists in the `Booking` module. Bookings are `submitted` and confirmed manually by clinic staff; this matches ADR-013's scope but should not be read as "coming soon" without a scheduled increment — see Roadmap Phase 1 for the Stripe Malaysia adapter's actual (currently unscheduled) gate. |
...
1. Schedule the ADR-008 Stripe Malaysia payment adapter — no payment code exists anywhere in `Booking` today; bookings remain manually confirmed by clinic staff. Do not treat this as "in progress" until it has an owner and a milestone.
