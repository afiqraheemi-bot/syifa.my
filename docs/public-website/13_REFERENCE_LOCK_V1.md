...
> **Patch (2026-08-08):** `--border-strong`'s literal value (this record freezes the role, not the literal hex — see the Border row below) was darkened from `#869c92` to `#749183` after direct WCAG 1.4.11 computation showed 2.88:1 against `surface-primary`, under the 3:1 non-text minimum, at real UI-component consumers (`.button--secondary`'s border, the booking-flow consent checkbox's border). `#749183` measures 3.38:1. Same hue family; the role's meaning, contrast guarantee, and consumer set are unchanged, so this is a Patch per the Change-governance rule below.

> **Patch (2026-08-08):** `--focus-ring` (#875c00) was tuned for light surfaces (5.7-5.9:1 there, still fine) but measured 1.04-2.86:1 — under WCAG 1.4.11/2.4.7's 3:1 minimum — against every template's dark surfaces, including `.booking-panel` (the Booking CTA — Essential 1.09:1, Dental 1.04:1) and `.skip-link` (the first focusable element on the page). Added a scoped `outline-color` override to `.skip-link:focus-visible`, `.site-footer :focus-visible`, `.public-section--contact :focus-visible`, and `.booking-panel :focus-visible` that reuses each template's own already-governed `accent-inverse` token (4.6-12.8:1 against every dark surface tested) — not a tenant `brand-*` token, so the "no template or tenant input may ever change focus visibility" rule in the Focus row above is preserved in spirit and mechanism; this strengthens focus visibility, it does not make it tenant-configurable. Encoded as a permanent regression test: `tests/Architecture/TemplateAccessibilityContrastArchitectureTest.php`.
...
| Focus | `focus-ring` (+ inline offset); dark-context outline color amended 2026-08-08 (see Patch note below) | Fixed platform value, `outline` styling that survives `forced-colors` | **Prohibited from tenant override** — no template or tenant input may ever change focus visibility | No — platform-fixed everywhere, including the dark-context amendment (uses each template's own governed `accent-inverse`, never a tenant `brand-*` token) |
