...
> **Website Service Presentation Authority amendment (2026-08-18):** `website_service_section_items` stores normalized Website/Section lineage, opaque Service identity, deterministic display order, and boolean featured state. Primary, order, composite-lineage, and partial featured uniqueness constraints preserve one item per Service and at most one featured item per Services Section. Existing latest-published references backfill in preserved order with featured false; immutable historic publication meaning is unchanged.
...
| **Booking** | **Business lifecycle state only (`submitted`, `confirmed`, `cancelled`, `completed`)** | **Not used — Booking must not use generic soft deletion; cancelled and completed outcomes remain fully queryable business states, never a hidden or flagged-deleted row** | Yes, for old completed/cancelled Bookings once outside the active operational window | Never for a Booking that reached an accepted state | Applies to the Booking Contact value object it carries, not to the Booking's own business facts (see next row) | Duration deferred; interacts with pending legal input on Booking retention; no-show is outside Phase 1 per ADR-013 |
