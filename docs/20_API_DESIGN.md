...
- Business Rules: Payment may only be initiated from a claimable CommercialOffer. A CommercialOffer claim is not payment success. ADR-008 uses Stripe-hosted Checkout for one-off MYR FPX/cards. Browser return is informational only; `succeeded` requires a signed webhook notification (or explicit reconciliation trigger) followed by authoritative Stripe API verification of reference, amount, currency, mode and status. A successful Payment does not by itself authorize a participant — it may cause Subscription and Entitlement transition only through Subscription's own approved commercial rules (18_AGGREGATE_DESIGN.md).
...
- Request Summary: CommercialOffer reference and approved method choice (`fpx` or `card`) only. The response supplies a Stripe-hosted Checkout action URL; raw payment instruments and provider secrets never transit this business API.
