...
final readonly class BookingStatusSummaryProvider
{
    ...
    public function provide(AuthorizationContext $context): DashboardSectionProjection
    {
        if ($context->tenantId === null) {
            throw new LogicException('Booking status summary requires a trusted Tenant identifier.');
        }

        $counts = $this->bookings->countByStatus($context->tenantId);

        return new DashboardSectionProjection('bookingStatusSummary', [
            ...
            'items' => array_map(
                ...
                BookingOverviewCriteria::statusOptions(),
            ),
        ]);
    }
}
