...
final readonly class WebsitePublicationContent
{
     ...
     */
    public function __construct(public array $contents, public array $renderabilityBySectionId)
    {
    ...
    public function capture(WebsiteSectionCollection $sections, PublicationId $publicationId, WebsiteId $websiteId, int $publishedVersion, WebsiteBranding $branding, DateTimeImmutable $at): array
    {
        ...
        foreach ($sections->sections() as $section) {
            ...
            $contact = $content instanceof ContactSectionContent ? new PublishedContactProjection($branding->contactEmail, $branding->contactPhone, $branding->address, $branding->socialLinks) : null;
            $canonical = $this->canonical($content, $contact);
            $encoded = json_encode($canonical, JSON_THROW_ON_ERROR | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
            $snapshots[] = new PublishedSectionContentSnapshot($publicationId, $websiteId, $section->id, $section->type, $publishedVersion, $content, hash('sha256', $encoded), $renderable, $at, 1, $contact);
        }
    ...
    private function canonical(WebsiteSectionContentInterface $content, ?PublishedContactProjection $contact): array
    {
        return match (true) {
            ...
            $content instanceof ContactSectionContent && $contact !== null => ['type' => SectionType::Contact->value, 'email' => $contact->email, 'phone' => $contact->phone, 'address' => $contact->address, 'socialLinks' => $contact->socialLinks],
