...
final class WebsiteSeoConfigurationPersistenceMapper
{
    ...
    {
        return new WebsiteSeoConfigurationStorageRecord($seo->websiteId->value, $seo->metaTitle(), $seo->metaDescription(), $seo->metaKeywords(), $seo->canonicalUrl(), $seo->robotsDirective()->value, $seo->openGraphTitle(), $seo->openGraphDescription(), $seo->openGraphImageReference()?->value, $seo->indexingEnabled(), $seo->createdAt, $seo->updatedAt(), $seo->version());
    }

    public function toDomain(WebsiteSeoConfigurationStorageRecord $record): WebsiteSeoConfiguration
    {
        return new WebsiteSeoConfiguration(new WebsiteId($record->websiteId), $record->metaTitle, $record->metaDescription, $record->metaKeywords, $record->canonicalUrl, RobotsDirective::fromStored($record->robotsDirective), $record->openGraphTitle, $record->openGraphDescription, $record->openGraphImageReference === null ? null : new AssetId($record->openGraphImageReference), $record->indexingEnabled, $record->domainCreatedAt, $record->domainUpdatedAt, $record->version);
    }
}
