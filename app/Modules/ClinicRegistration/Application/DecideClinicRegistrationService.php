...
final readonly class DecideClinicRegistrationService
{
    ...
    public function execute(DecideClinicRegistrationCommand $command): int
    {
        return $this->transaction->run(function () use ($command): int {
            ...
            if ($registration->version() !== $command->expectedVersion) {
                throw new InvalidClinicRegistrationTransitionException('Clinic registration changed. Refresh before recording a decision.');
            }

            $outcome = RegistrationDecisionOutcome::tryFrom($command->outcome)
                ?? throw new InvalidClinicRegistrationTransitionException('Registration decision outcome is not supported.');
            $registration->decide(
                $command->decisionId,
                $outcome,
                $command->reasonCategory,
                $command->correctionInstructions,
                $command->reviewerPlatformIdentityId,
                $command->occurredAt,
            );
            $this->registrations->save($registration);
            $this->audit->record(
                self::deterministicId('decision', $command->decisionId),
                $command->reviewerPlatformIdentityId,
