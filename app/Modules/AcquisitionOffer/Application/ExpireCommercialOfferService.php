...
use App\Modules\AcquisitionOffer\Contracts\Commands\ExpireCommercialOfferCommand;
...
final readonly class ExpireCommercialOfferService
{
    ...
    public function execute(ExpireCommercialOfferCommand $command): CommercialOfferData
    {
        return $this->transactions->run(function () use ($command): CommercialOfferData {
            ...
            $offer->expire($command->occurredAt);
            $this->offers->save($offer);
            $this->audit->recordForPlatformIdentity('commercial.offer.expire', $offer, $command->occurredAt, $command->correlationId);
