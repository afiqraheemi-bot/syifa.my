...
final readonly class PlanOfferingAuditTrail
{
    private const string TARGET_TYPE = 'commercial_catalogue.plan_offering';
    ...
    public function record(
        ...
        $this->auditEntries->record(new AuditEntryData(
            self::auditEntryId($action, $occurredAt, $actorPlatformIdentityId, $correlationId, $offering->id->value),
            ...
            null,
            $action,
            new AuditTargetData(self::TARGET_TYPE, $offering->id->value),
            new AuditOutcomeData(AuditOutcomeType::Succeeded->value, null),
            $correlationId,
            [
                'resource_type' => self::TARGET_TYPE,
                'resource_label' => $offering->id->value,
                'target_label' => sprintf(
                    'plan_id=%s;previous_version=%d;resulting_version=%d;previous_status=%s;resulting_status=%s',
                    $offering->planId->value,
                    $previousVersion,
                    $offering->version(),
                    $previousStatus,
