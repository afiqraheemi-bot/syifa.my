 ...
 * Plan additionally consumes it as Domain lifecycle time (see Plan::retire()).
 */
final readonly class RetirePlanService
{
    private const string AUDIT_ACTION = 'commercial_catalogue.plan.archive';
    ...
    public function execute(RetirePlanCommand $command): Plan
    {
        $plan = $this->requirePlan($command->planId);
        $this->assertExpectedVersion($plan, $command->expectedVersion);

        $updated = $plan->retire(self::instant($command->occurredAt));
