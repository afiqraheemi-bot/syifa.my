...
final readonly class BookingHistoryEntry
{
    ...
    public static function submitted(string $id, Booking $booking, DateTimeImmutable $at): self
    {
        return new self($id, $booking->tenantId->value, $booking->id->value, BookingHistoryEventType::Submitted, BookingActorType::PublicVisitor, null, $at, self::snapshot($booking->scheduledAppointment()));
    }
    ...
    public static function reconstitute(string $id, string $tenantId, string $bookingId, string $eventType, string $actorType, ?string $actorId, DateTimeImmutable $occurredAt, array $payload): self
    {
        ...
        $required = match ($type) {
            BookingHistoryEventType::Submitted => ['local_date', 'local_start', 'local_end', 'timezone', 'starts_at_utc', 'ends_at_utc', 'duration_minutes'],
