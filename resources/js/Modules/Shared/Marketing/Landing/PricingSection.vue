...
const copy = computed(() =>
    props.lang === 'en'
        ? {
              ...
              compareBody: 'Both cover the essentials. Choose Standard when you want more automation and a professional domain.',
              ...
              basicAudience: 'Clinics that need a professional website and organised online bookings.',
              ...
              basicFeatures: ['Managed clinic website', 'Online booking system', 'Content and branding management'],
              standardFeatures: ['SYIFA AI Assistant', 'Your own custom domain', 'Managed website and online booking'],
              trialFeatures: ['Explore the managed website', 'Test the online booking flow', 'No charge during the trial'],
          }
        : {
              ...
              compareBody: 'Kedua-duanya merangkumi keperluan utama. Pilih Standard apabila anda mahukan lebih automasi dan domain profesional.',
              ...
              basicAudience: 'Klinik yang perlukan website profesional dan tempahan online yang teratur.',
              standardAudience: 'Klinik yang bersedia mengautomasikan kerja dan mengukuhkan jenama online.',
              trialAudience: 'Klinik yang mahu mencuba aliran utama sebelum melanggan.',
              basicFeatures: ['Website klinik terurus', 'Sistem tempahan online', 'Pengurusan kandungan dan penjenamaan'],
              standardFeatures: ['SYIFA AI Assistant', 'Custom domain milik anda', 'Website terurus dan tempahan online'],
              trialFeatures: ['Cuba website terurus', 'Uji aliran tempahan online', 'Tiada caj sepanjang percubaan'],
          },
);
...
<template>
    <section id="pricing" class="anchor-section bg-emerald-50/55 py-20 sm:py-28">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-10">
            ...
            <div
                v-if="packages.some((item) => packageKind(item) === 'basic') && packages.some((item) => packageKind(item) === 'standard')"
            ...
            >
                <div class="border-b border-emerald-100 px-5 py-5 text-center sm:px-8">
                    ...
                    <p class="mx-auto mt-1 max-w-2xl text-sm leading-6 text-slate-600">{{ copy.compareBody }}</p>
                </div>
                <div class="grid md:grid-cols-[1fr_auto_1fr] md:items-stretch">
                    <div class="p-5 sm:p-6">
                        <p class="text-xs font-black tracking-[0.14em] text-slate-500 uppercase">Basic</p>
                    ...
                    <div class="relative flex items-center justify-center px-5 md:px-0">
                        ...
                        <span class="absolute flex size-8 items-center justify-center rounded-full bg-emerald-700 text-sm font-black text-white ring-4 ring-white" aria-hidden="true">+</span>
                    </div>
                    <div class="bg-emerald-50/70 p-5 sm:p-6">
                        <p class="text-xs font-black tracking-[0.14em] text-emerald-700 uppercase">Standard</p>
                        <p class="mt-2 font-bold text-slate-950">{{ copy.standardLabel }}</p>
                        <p class="mt-1 text-sm font-semibold text-emerald-800">{{ copy.standardSummary }}</p>
                    </div>
                </div>
            </div>

            <div v-if="packages.length" class="mt-10 grid items-stretch gap-6 md:grid-cols-2 lg:grid-cols-3">
                <article
                    ...
                    :class="isFeatured(item) ? 'border-emerald-500 ring-2 ring-emerald-100' : 'border-emerald-100'"
                >
                    ...
                    <p class="mt-3 min-h-12 text-sm leading-6 text-slate-600">{{ item.description }}</p>
                    <div class="mt-5 rounded-xl border border-slate-100 bg-slate-50/80 p-4">
                        <p class="text-[11px] font-black tracking-[0.12em] text-slate-500 uppercase">{{ copy.bestFor }}</p>
                        <p class="mt-1.5 text-sm font-semibold leading-5 text-slate-800">{{ audience(item) }}</p>
                    </div>
                    <div class="mt-7 border-t border-slate-100 pt-6">
                        <p class="text-4xl font-black tracking-tight text-slate-950">{{ item.price }}</p>
                        <p class="mt-1 text-sm font-medium text-slate-500">
                            {{ item.isTrial ? copy.noCharge : `${item.billingCycle} · ${copy.cycle}` }}
                        </p>
                    </div>
                    <div class="mt-6 flex-1">
                        <p class="text-xs font-black tracking-[0.12em] text-slate-500 uppercase">
                            {{ packageKind(item) === 'standard' ? copy.everythingBasic : copy.includes }}
                        </p>
                        <ul class="mt-3 space-y-3">
                            <li v-for="feature in features(item)" :key="feature" class="flex gap-2.5 text-sm font-semibold leading-5 text-slate-700">
                                <span class="mt-0.5 flex size-5 shrink-0 items-center justify-center rounded-full bg-emerald-100 text-xs font-black text-emerald-700" aria-hidden="true">✓</span>
                                <span>{{ feature }}</span>
                            </li>
                        </ul>
                    </div>
            ...
            <p v-else class="mx-auto mt-10 max-w-xl rounded-2xl border border-emerald-100 bg-white p-6 text-center text-slate-600 shadow-sm">{{ copy.empty }}</p>
        </div>
    </section>
</template>
